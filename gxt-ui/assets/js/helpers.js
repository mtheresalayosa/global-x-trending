...
const mapTweets = (tweets, city) => {
  const tindex = tweets.map(e => e.name).indexOf(city.properties.sov0name);
  console.log(city.properties.sov0name,'tindex=',tindex)
  let rtweets = tweets[tindex];
  // let rtweets = tweets.find((t)=> t.name == city.properties.sov0name)
  if(tindex == -1) {
    rtweets = {
      "name": city.properties.sov0name,
      "woeid": 0,
      "keyword": "",
      "tweet_volume": 0 
    }
...
const geoTweets = (tweets, geoCity) => {
  let geoCityDuplicate = geoCity;
  let cities = geoCityDuplicate.features;
  cities.map( (city) => {
    const tindex = tweets.map(e => e.name).indexOf(city.properties.sov0name);
    if(tindex == -1) {
      city.properties.tweet_volume = 0;
      city.properties.keyword = "";
    } else {
      city.properties.tweet_volume = tweets[tindex].tweet_volume;
      city.properties.keyword = tweets[tindex].keyword;
    }
  });
  return geoCityDuplicate;
}
