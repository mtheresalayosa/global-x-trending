...
function enableRotation(d_projection, canvas) {
	d3.timer(function (elapsed) {
		d_projection.rotate([config.speed * elapsed - 120, config.verticalTilt, config.horizontalTilt]);
    const path = d3.geoPath().projection(d_projection)
...
/* Load the data */
const promises = [
  d3.json('/assets/data/ne_50m_admin_0_countries.json'),
  d3.json('/assets/data/ne_10m_populated_places_simple.geojson'),
  d3.json('/assets/data/trending.json'),
];
...
Promise.all(promises)
  .then((data) => {
    ...
    const geoCitiesData = data[1];
    const trendingData = data[2];

    const citiesData = geoTweets(trendingData, geoCitiesData);
    ...
    drawGraph(projection, path, countriesData, citiesData, trendingData);
    // insert rotation
    // enableRotation(projection, context);
  })
...
const render = ({
  ...
  cityPath.pointRadius(
    (d) => 
      popScale(d.properties.tweet_volume) / (transform.k < 1 ? 1 : transform.k)
  );
  ...
  context.fillStyle = '#3C4E32';
  context.strokeStyle = '#77B0AA';
  ...
  citiesData.features.forEach((city) => {
    ...
    context.fillStyle = colorScale(city.properties.tweet_volume);
    context.fill();
  });
  ...
  if (selected && transform.k === 1) {
    ...
    const name = `${selected.properties.keyword} - ${formatNumber(selected.properties.tweet_volume)}`;
...
const drawGraph = (projection, path, countriesData, citiesData, trendingData) => {
